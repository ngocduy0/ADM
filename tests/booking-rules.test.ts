...
test('venue metadata update allows unchanged legacy duplicate table names', () => {
  const legacy = { ...venue, preferredTables: [venue.preferredTables[0], { ...venue.preferredTables[0], id: 'table-2' }] };
  const result = validateVenueUpdate({ ...legacy, shortDescription: 'Updated metadata only' }, legacy);
  assert.equal(result.valid, true, JSON.stringify(result.issues));

  const changedTables = { ...legacy, preferredTables: legacy.preferredTables.map((table, index) => index === 1 ? { ...table, capacity: 8 } : table) };
  const changedResult = validateVenueUpdate(changedTables, legacy);
  assert.equal(changedResult.valid, false);
  assert.ok(changedResult.issues.some((issue) => issue.field === 'preferredTables'));
});
