...
export function VenuesPage() {
  ...
  return (
    <div className="pb-10">
      ...
      {rows.length ? (
        <>
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{rows.map((venue) => {
          ...
          <Pagination page={currentPage} totalItems={filtered.length} pageSize={PAGE_SIZE} onPageChange={setPage} itemLabel="địa điểm" />
        </>
      ) : <EmptyState icon={MapPin} title="Chưa có địa điểm" description="Thêm địa điểm đầu tiên để bắt đầu nhận booking." action={<Button onClick={() => setCreateOpen(true)}><Plus size={18} />Thêm địa điểm</Button>} />}
