...
export function TablesPage() {
  ...
  const rows = tables.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);

  useEffect(() => setPage(1), [venueId, searchQuery]);
  useEffect(() => {
  ...
  return (
    <div className="pb-10">
      ...
      <Card className="mb-5 flex flex-col gap-3 p-4 sm:flex-row sm:items-center"><label className="text-xs font-extrabold uppercase tracking-wider text-slate-400">Địa điểm</label><select value={activeVenue?.id || ''} onChange={(event) => setVenueId(event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-extrabold outline-none focus:border-[#1F3A8A] sm:min-w-[240px] sm:w-auto">{venues.map((venue) => <option key={venue.id} value={venue.id}>{venue.name}</option>)}</select><div className="flex items-center gap-4 text-xs font-bold text-slate-500 sm:ml-auto"><span>{tables.length} bàn</span><span>{activeVenue?.tableZones?.length || 0} khu</span></div></Card>
      {activeVenue && rows.length ? (
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{rows.map((table) => {
          ...
          <Pagination page={currentPage} totalItems={tables.length} pageSize={PAGE_SIZE} onPageChange={setPage} itemLabel="bàn" />
        </>
      ) : <EmptyState icon={TableProperties} title={activeVenue ? 'Địa điểm chưa có bàn' : 'Chưa có địa điểm'} description={activeVenue ? 'Thêm bàn mới hoặc mở chỉnh sửa địa điểm để xây dựng sơ đồ đầy đủ.' : 'Hãy tạo địa điểm trước khi quản lý bàn.'} action={activeVenue ? <Button onClick={() => setCreateOpen(true)}><Plus size={18} />Thêm bàn</Button> : undefined} />}
