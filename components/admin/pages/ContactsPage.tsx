...
export function ContactsPage() {
  const { settings, saveSettings, uploadMedia, deleteMedia, saving, showToast } = useAdminData();
  ...
  const uploadIcon = async (channel: ContactChannel, file?: File) => {
    ...
    try {
      const uploaded = await uploadMedia(file, `brand/contacts/${channel.id}`);
      sessionUploadedPaths.current.add(uploaded.path);
      queueCleanup(channel.iconPath);
  ...
  const save = async () => {
    try {
      ...
      const activePaths = new Set(saved.contactChannels.map((channel) => channel.iconPath).filter(Boolean));
      ...
      if (failed.length) {
        showToast('error', `Kênh liên hệ đã lưu nhưng còn ${failed.length} icon cũ chưa xóa được.`);
      }
    } catch {
  ...
  const removeChannel = (channel: ContactChannel) => {
    queueCleanup(channel.iconPath);
    setChannels(draft.contactChannels.filter((item) => item.id !== channel.id));
  };
  ...
  return (
    <div className="pb-10">
      <PageHeader
        ...
        actions={<Button onClick={() => void save()} disabled={saving || Boolean(uploadingId)}><Save size={18} />{saving ? 'Đang lưu...' : 'Lưu thay đổi'}</Button>}
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <Card key={channel.id} className="p-5">
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start">
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-2 lg:w-48 lg:justify-end">
                  ...
                  <button onClick={() => removeChannel(channel)} className="grid h-9 w-9 place-items-center rounded-xl text-red-500 hover:bg-red-50" aria-label="Xóa"><Trash2 size={16} /></button>
                </div>
              </div>
            </Card>
          ))}
        ...
        <Card className="h-fit p-5 xl:sticky xl:top-24">
          ...
          <p className="mt-4 text-xs font-medium leading-5 text-slate-500">Sau khi lưu, thứ tự, trạng thái hiển thị, đường dẫn và icon được dùng chung cho thanh liên hệ nhanh trên website.</p>
