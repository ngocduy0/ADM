...
export function DataFilesPage() {
  ...
  return (
    <div className="pb-10">
      <PageHeader title="Tệp dữ liệu" description="Nhập, xuất, sao lưu và dọn dữ liệu vận hành." />
      <div className="mb-8 grid gap-5 md:grid-cols-3">
        ...
        <FileCard icon={<RefreshCcw size={24} />} title="Hệ thống" description="Sao lưu đầy đủ hoặc xóa dữ liệu vận hành." actions={<><button onClick={exportJson}>Sao lưu JSON<FileJson size={15} /></button><button className="!text-red-600 hover:!bg-red-50" onClick={() => setResetOpen(true)}>Xóa booking, khách & yêu cầu<AlertTriangle size={15} /></button></>} tone="danger" />
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          >
            <div className="flex min-h-[360px] flex-col items-center justify-center rounded-[26px] bg-slate-50/60 px-8 py-14"><span className="grid h-20 w-20 place-items-center rounded-full bg-[#D9DFF5] text-[#1F3A8A]"><UploadCloud size={38} /></span><h2 className="mt-6 text-2xl font-black">Tải lên tệp dữ liệu</h2><p className="mt-2 max-w-md text-sm font-medium leading-6 text-slate-500">Kéo thả file backup JSON hoặc CSV booking vào đây, hoặc nhấn để chọn từ máy tính.</p><div className="mt-7 flex flex-wrap justify-center gap-3"><span className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black"><CheckCircle2 size={14} className="mr-1 inline text-[#1F3A8A]" />Hỗ trợ .json, .csv</span><span className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black"><CheckCircle2 size={14} className="mr-1 inline text-[#1F3A8A]" />Đồng bộ dữ liệu</span></div></div>
          </button>
      ...
      <ConfirmDialog open={resetOpen} title="Xóa dữ liệu vận hành?" description="Chỉ booking, khách hàng, yêu cầu đặt chỗ và yêu cầu liên hệ sẽ bị xóa. Địa điểm, hình ảnh, video, bàn, khu và sơ đồ được giữ nguyên. Nên tải backup JSON trước khi tiếp tục." confirmLabel="Xóa dữ liệu vận hành" onClose={() => setResetOpen(false)} onConfirm={resetData} />
    </div>
  );
}
