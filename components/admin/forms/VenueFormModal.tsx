...
import { Film, ImagePlus, LayoutTemplate, Loader2, Plus, Trash2, UploadCloud } from 'lucide-react';
...
export function VenueFormModal({ open, venue, onClose }: { open: boolean; venue?: Venue | null; onClose: () => void }) {
  ...
  const uploadVenueVideo = async (file?: File) => {
    if (!file) return;
    if (!file.type.startsWith('video/')) return showToast('error', 'Vui lòng chọn file video.');
    setUploading(true);
    try {
      const uploaded = await uploadMedia(file, 'venues/videos');
      const oldPath = draft.videoUrl ? draft.mediaPaths?.[draft.videoUrl] : undefined;
      ...
      setDraft((current) => ({
        ...
        mediaPaths: { ...(current.mediaPaths || {}), [uploaded.url]: uploaded.path },
      }));
      showToast('success', 'Đã upload video địa điểm lên Cloudinary.');
    } catch (error) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const activeMediaUrls = new Set([...images, draft.videoUrl || ''].filter(Boolean));
    ...
    try {
      ...
      if (failedCount) showToast('error', `Địa điểm đã lưu nhưng còn ${failedCount} ảnh cũ chưa xóa được.`);
      setSessionUploadedPaths([]);
      onClose();
    } catch {
      ...
      <Modal open={open} title={venue ? 'Chỉnh sửa địa điểm' : 'Thêm địa điểm'} description="Thông tin hiển thị ở website người dùng và toàn bộ quy trình booking." onClose={closeWithoutSaving} size="xl" footer={<><Button variant="secondary" onClick={closeWithoutSaving}>Hủy</Button><Button type="submit" form="venue-form" disabled={saving || uploading}>{saving ? 'Đang lưu...' : 'Lưu địa điểm'}</Button></>}>
        <form id="venue-form" onSubmit={submit} className="space-y-7">
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <FormField label="Video địa điểm (Cloudinary)"><div className="flex gap-2"><input className={inputClass} value={draft.videoUrl || ''} onChange={(event) => setDraft({ ...draft, videoUrl: event.target.value })} placeholder="https://res.cloudinary.com/.../venue.mp4" /><Button variant="outline" onClick={() => videoInput.current?.click()} disabled={uploading} className="shrink-0">{uploading ? <Loader2 size={16} className="animate-spin" /> : <Film size={16} />}Upload</Button><input ref={videoInput} type="file" accept="video/mp4,video/webm,video/quicktime" className="hidden" onChange={(event) => uploadVenueVideo(event.target.files?.[0])} /></div></FormField>
          ...
          <section className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div><h3 className="font-extrabold text-slate-900">Hình ảnh địa điểm</h3><p className="mt-1 text-xs font-medium text-slate-500">Ảnh đầu tiên là ảnh đại diện. Tất cả ảnh mới được upload trực tiếp lên Cloudinary.</p></div>
