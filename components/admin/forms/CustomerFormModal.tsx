...
export function CustomerFormModal({
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (draft.fullName.trim().length < 2)
      return showToast("error", "Tên khách hàng cần ít nhất 2 ký tự.");
    if (!isVietnamesePhone(draft.phoneNumber))
  ...
  return (
    <Modal
      ...
      description="Hồ sơ riêng của khách và phân hạng chăm sóc."
      onClose={onClose}
      size="md"
      footer={
        <>
    ...
    >
      <form id="customer-form" onSubmit={submit} className="space-y-5">
        ...
        <FormField
          label="Phân hạng"
          hint="Hệ thống tự tính theo chi tiêu hoàn tất và hệ số khu vực."
        >
          <input
            className={inputClass}
        ...
        <FormField label="Ghi chú">
          <textarea
            ...
            placeholder="Sở thích, lịch sử chăm sóc, lưu ý riêng..."
          />
        </FormField>
      </form>
    </Modal>
