...
async function uploadAdminMedia(file: File, folder: 'homepage/banner' | 'homepage/banner/posters') {
  ...
  form.append('public_id', String(signed.publicId));
  form.append('overwrite', 'false');

  const response = await fetch(String(signed.uploadUrl), { method: 'POST', body: form });
  const json = await response.json().catch(() => null);
  if (!response.ok || !json?.secure_url || !json?.public_id) {
    throw new Error(json?.error?.message || 'Cloudinary upload thất bại.');
  }

  const resourceType = String(json.resource_type || signed.resourceType || 'video');
  const originalUrl = String(json.secure_url);
  return {
    url: resourceType === 'image' ? originalUrl.replace('/image/upload/', '/image/upload/f_auto,q_auto:eco/') : originalUrl,
    path: `cloudinary://${resourceType}/${String(json.public_id)}`,
  };
}

async function deleteAdminMedia(path?: string) {
  if (!path) return;
...
export default function BannerVideoManager({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const uploadBanner = async (files: FileList | null) => {
    ...
    if (file.size > MAX_BANNER_VIDEO_BYTES) {
      setMessage({ kind: 'error', text: `Video quá nặng (${formatMb(file.size)}). Tối đa hiện tại là 80MB. Nên nén video banner còn khoảng 2–8MB để tải nhanh trên mobile.` });
      return;
    }
    ...
    try {
      ...
      const oldVideoPath = settings.heroVideoPath;
      const oldPosterPath = settings.heroPosterPath;
      const uploadedVideo = await uploadAdminMedia(file, 'homepage/banner');

      let posterUrl = settings.heroPosterUrl || '';
      let posterPath = settings.heroPosterPath || '';

      if (posterFile) {
        const uploadedPoster = await uploadAdminMedia(posterFile, 'homepage/banner/posters');
      ...
      await Promise.allSettled([
        oldVideoPath && oldVideoPath !== saved.heroVideoPath ? deleteAdminMedia(oldVideoPath) : Promise.resolve(),
        oldPosterPath && oldPosterPath !== saved.heroPosterPath ? deleteAdminMedia(oldPosterPath) : Promise.resolve(),
      ]);
      setMessage({ kind: 'success', text: 'Đã cập nhật video banner homepage trên Cloudinary.' });
    } catch (error) {
  ...
  const clearBanner = async () => {
    ...
    try {
      ...
      setSettings(saved);
      await Promise.allSettled([deleteAdminMedia(oldVideoPath), deleteAdminMedia(oldPosterPath)]);
      setMessage({ kind: 'success', text: 'Đã tắt video banner và xóa media Cloudinary cũ.' });
    } catch (error) {
            ...
            <div className="grid gap-0 lg:grid-cols-[1fr_360px]">
              ...
              <div className="space-y-4 p-6 text-[#1D1D1F]">
                <div className="rounded-3xl border border-[#E8E8ED] bg-[#F5F5F7] p-4 text-sm leading-relaxed text-[#515154]">
                  ...
                  <p className="mt-2">Video banner nên là MP4/WebM, ưu tiên 720p hoặc 1080p và nén còn khoảng 2–8MB. Hệ thống chỉ lưu URL/path, không lưu base64 vào database.</p>
                  <p className="mt-2">Khi lưu video mới thành công, file banner cũ trên Cloudinary sẽ được xóa để tránh tốn dung lượng.</p>
                </div>
