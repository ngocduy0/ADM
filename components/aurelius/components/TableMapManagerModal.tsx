...
export default function TableMapManagerModal({
  ...
  const [mobilePanel, setMobilePanel] = useState<"catalog" | "map" | "inspector">("map");
  ...
  const catalogTotalPages = Math.max(1, Math.ceil(visibleTables.length / catalogPageSize));
  const currentCatalogPage = Math.min(catalogPage, catalogTotalPages);
  const catalogTables = visibleTables.slice(
  ...
  return (
    <div className={embedded ? "h-full min-h-0 bg-[#F5F5F7]" : "fixed inset-0 z-[100] bg-black/55 p-3 backdrop-blur-md md:p-6"}>
      <div className={embedded ? "flex h-full min-h-0 w-full flex-col overflow-hidden bg-[#F5F5F7]" : "mx-auto flex h-full max-w-[1520px] flex-col overflow-hidden rounded-[30px] bg-[#F5F5F7] shadow-2xl"}>
        <header className="hidden flex-wrap items-center justify-between gap-3 border-b border-[#E5E5EA] bg-white px-5 py-4 lg:flex">
          <div>
        ...
        <div className="flex shrink-0 items-center gap-1.5 border-b border-[#E5E5EA] bg-white px-2 py-2 lg:hidden">
          <button type="button" onClick={() => setMobilePanel("catalog")} className={`flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl text-[11px] font-black ${mobilePanel === "catalog" ? "bg-[#1F3A8A] text-white" : "bg-[#F5F5F7] text-[#555]"}`}><Layers3 className="h-4 w-4" />Danh sách</button>
          <button type="button" onClick={() => setMobilePanel("map")} className={`flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl text-[11px] font-black ${mobilePanel === "map" ? "bg-[#1F3A8A] text-white" : "bg-[#F5F5F7] text-[#555]"}`}><Grid3X3 className="h-4 w-4" />Sơ đồ</button>
          <button type="button" onClick={() => setMobilePanel("inspector")} className={`flex h-10 flex-1 items-center justify-center gap-1.5 rounded-xl text-[11px] font-black ${mobilePanel === "inspector" ? "bg-[#1F3A8A] text-white" : "bg-[#F5F5F7] text-[#555]"}`}><Palette className="h-4 w-4" />Chi tiết</button>
          {onSave ? <button type="button" onClick={onSave} disabled={saving} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-600 text-white disabled:opacity-50" aria-label="Lưu sơ đồ"><Save className="h-4 w-4" /></button> : null}
          <button type="button" onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-[#E5E5EA] bg-white text-[#1D1D1F]" aria-label="Quay lại"><ArrowLeft className="h-4 w-4" /></button>
        </div>

        <div className={embedded ? "grid min-h-0 flex-1 grid-cols-1 grid-rows-[minmax(0,1fr)] gap-2 overflow-hidden p-2 lg:grid-cols-[300px_minmax(0,1fr)_340px] lg:gap-3 lg:p-3" : "grid min-h-0 flex-1 grid-cols-1 grid-rows-[minmax(0,1fr)] gap-2 overflow-hidden p-2 lg:grid-cols-[330px_minmax(0,1fr)_380px] lg:gap-4 lg:p-4"}>
          <aside className={`${mobilePanel === "catalog" ? "block" : "hidden"} min-h-0 overflow-y-auto rounded-[20px] border border-[#E5E5EA] bg-white p-3 lg:block lg:rounded-[24px] lg:p-4`}>
            ...
            {tab !== "style" && (
              <div className="mb-4 rounded-2xl border border-[#E5E5EA] bg-[#FBFBFD] p-3">
                ...
                <div className="mt-2 flex flex-col gap-2">
                  <button
                    type="button"
                    onClick={() => { setSelectedZoneId("ALL"); setCatalogPage(1); }}
                  ...
                  {cleanZones.map((zone) => (
                    <button
                      key={zone.id}
                      type="button"
                      onClick={() => { setSelectedZoneId(zone.id); setCatalogPage(1); }}
            ...
            {tab === "tables" && (
              <div className="space-y-2">
                ...
                {visibleTables.length > catalogPageSize ? (
                  <div className="sticky bottom-0 mt-3 flex items-center justify-between gap-2 rounded-2xl border border-[#E5E5EA] bg-white/95 p-2 shadow-lg backdrop-blur">
                    <button
                      type="button"
                      disabled={currentCatalogPage <= 1}
                      onClick={() => setCatalogPage((value) => Math.max(1, value - 1))}
                      className="h-9 rounded-xl border border-[#E5E5EA] px-3 text-[11px] font-black disabled:opacity-35"
                    >
                      Trước
                    </button>
                    <span className="text-[11px] font-black text-[#86868B]">
                      {currentCatalogPage}/{catalogTotalPages} · {visibleTables.length} bàn
                    </span>
                    <button
                      type="button"
          ...
          <main className={`${mobilePanel === "map" ? "block" : "hidden"} min-h-0 overflow-y-auto rounded-[20px] border border-[#E5E5EA] bg-white p-3 lg:block lg:rounded-[24px] lg:p-4`}>
            ...
            <div className="overflow-auto rounded-[20px] border border-[#20242E] bg-[#080A0F] p-2 sm:p-4 lg:rounded-[26px]">
              <div
                ref={mapRef}
          ...
          <aside className={`${mobilePanel === "inspector" ? "block" : "hidden"} min-h-0 overflow-y-auto rounded-[20px] border border-[#E5E5EA] bg-white p-3 lg:block lg:rounded-[24px] lg:p-4`}>
