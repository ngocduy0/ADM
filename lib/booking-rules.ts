...
export function validateReservation(
  ...
  options: {
    now?: Date;
    existing?: ReservationRequest | null;
  ...
  const minimumLeadMinutes = Math.max(0, options.minimumLeadMinutes || 0);
  ...
  if (scheduledAt) {
    ...
    const minimumAllowedTime = minimumLeadMinutes
      ? now.getTime() + minimumLeadMinutes * 60_000
      : now.getTime() - 60_000;
    if (
      (!existing || changedSchedule) &&
      scheduledAt.getTime() < minimumAllowedTime
    ) {
      issues.push({
        field: "date",
        message: minimumLeadMinutes
          ? `Booking công khai cần được tạo trước ít nhất ${minimumLeadMinutes} phút.`
          : "Không thể tạo hoặc chuyển booking về thời điểm đã qua.",
      });
    }
  ...
  if (table) {
    ...
    if (table.status === "RESERVED")
      issues.push({
        field: "preferredTableId",
        message: "Bàn/phòng này đang được vô hiệu hóa hoặc đã giữ chỗ.",
      });
...
export function validateVenueUpdate(venue: Venue, existing?: Venue | null): ValidationResult {
  const validation = validateVenue(venue);
  if (!existing) return validation;

  // ADM Club và một số dữ liệu legacy có thể đã chứa tên bàn trống/trùng từ trước.
  // Khi admin chỉ sửa thông tin địa điểm hoặc media, không chặn thao tác bởi phần
  // sơ đồ bàn không thay đổi. Nếu danh sách bàn được chỉnh sửa, validation đầy đủ
  // vẫn được áp dụng như bình thường.
  const currentTables = JSON.stringify(existing.preferredTables || []);
  const nextTables = JSON.stringify(venue.preferredTables || []);
  if (currentTables !== nextTables) return validation;

  const issues = validation.issues.filter((issue) => issue.field !== "preferredTables");
  return { valid: issues.length === 0, issues };
}
