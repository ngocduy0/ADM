...
import { BookingStatus, Customer, ReservationRequest, Venue, type HomepageReel } from '@/components/aurelius/types';
import {
...
function buildVenueDescription(venue: Venue) {
  ...
  const meta = `${VENUE_META_MARKER}${JSON.stringify({
    mediaPaths: venue.mediaPaths || undefined,
...
export async function incrementVenueViewCountFast(id: string) {
  ...
  const description = buildVenueDescription({
    ...
    image: '',
    images: [],
    mediaPaths: (currentMeta.mediaPaths || undefined) as Venue['mediaPaths'],
...
export async function updateVenueReelsFast(
  venueId: string,
  reels: NonNullable<Venue['reels']>,
  maxTotalReels = 10,
): Promise<NonNullable<Venue['reels']>> {
  const supabase = getSupabaseAdminClient();
  const rows = await checked(
    supabase.from('Venue').select('id,description'),
    'read Venue reels',
  ) as Array<{ id: string; description?: string | null }>;

  const row = rows.find((item) => item.id === venueId);
  if (!row) throw new Error('Venue not found');

  const otherReelCount = rows.reduce((total, item) => {
    if (item.id === venueId) return total;
    return total + (splitDescription(item.description).reels || []).length;
  }, 0);
  if (otherReelCount + reels.length > maxTotalReels) {
    throw new Error(`Hệ thống chỉ cho phép tối đa ${maxTotalReels} reels trên toàn bộ địa điểm.`);
  }

  const descriptions = splitDescription(row.description);
  const currentMeta = descriptions.meta || {};
  const normalizedReels = reels.map((reel, index) => ({
    ...reel,
    venueId,
    instagramUrl: String(reel.instagramUrl || '').trim(),
    order: Number.isFinite(Number(reel.order)) ? Number(reel.order) : index,
    isActive: reel.isActive !== false,
    placement: reel.placement || 'HOME_FEED',
  }));
  ...
  const description = buildVenueDescription({
    ...
    viewCount: Math.max(0, Number(currentMeta.viewCount || 0)),
    preferredTables: [],
    rating: Number(currentMeta.rating) || 4.8,
    reviewsCount: Math.max(0, Number(currentMeta.reviewsCount || 0)),
  ...
  await checked(
    supabase.from('Venue').update({ description }).eq('id', venueId),
    'update Venue reels',
  );
  invalidateConciergeCache();
...
export type ReelOrderInput = { venueId: string; reelId: string };

export async function reorderVenueReelsFast(
  ordered: ReelOrderInput[],
  maxTotalReels = 10,
): Promise<Array<ReelOrderInput & { order: number }>> {
  const supabase = getSupabaseAdminClient();
  const rows = await checked(
    supabase.from('Venue').select('id,description'),
    'read Venue reels for global reorder',
  ) as Array<{ id: string; description?: string | null }>;

  const existingKeys = new Set<string>();
  for (const row of rows) {
    for (const reel of splitDescription(row.description).reels || []) {
      existingKeys.add(`${row.id}::${reel.id}`);
    }
  }

  if (existingKeys.size > maxTotalReels || ordered.length > maxTotalReels) {
    throw new Error(`Hệ thống chỉ cho phép tối đa ${maxTotalReels} reels trên toàn bộ địa điểm.`);
  }
  if (ordered.length !== existingKeys.size) {
    throw new Error('Danh sách sắp xếp Reel không khớp dữ liệu hiện tại. Hãy tải lại trang.');
  }

  const orderMap = new Map<string, number>();
  ordered.forEach((item, order) => {
    const key = `${item.venueId}::${item.reelId}`;
    if (!existingKeys.has(key)) throw new Error('Có Reel không còn tồn tại. Hãy tải lại trang.');
    if (orderMap.has(key)) throw new Error('Danh sách sắp xếp Reel bị trùng nội dung.');
    orderMap.set(key, order);
  });

  const updates = rows.flatMap((row) => {
    const descriptions = splitDescription(row.description);
    const reels = descriptions.reels || [];
    if (!reels.length) return [];
    const currentMeta = descriptions.meta || {};
    const normalizedReels: HomepageReel[] = reels
      .map((reel, index) => ({
        ...reel,
        venueId: row.id,
        order: orderMap.get(`${row.id}::${reel.id}`) ?? index,
      }))
...
export async function clearLegacySupabaseMediaFast() {
  const supabase = getSupabaseAdminClient();
  const [venueRows, imageRows, settingRow] = await Promise.all([
    checked(supabase.from('Venue').select('id,description'), 'read Venue legacy media'),
    checked(supabase.from('VenueImage').select('id,imageUrl,venueId'), 'read VenueImage legacy media'),
    checked(supabase.from('SiteSetting').select('key,value').eq('key', 'site').maybeSingle(), 'read SiteSetting legacy media'),
  ]) as [
    Array<{ id: string; description?: string | null }>,
    Array<{ id: string; imageUrl?: string | null; venueId?: string | null }>,
...
export async function readHomepageVenues(): Promise<Venue[]> {
  ...
  const venues: Venue[] = venuesDb.map((venue) => {
    ...
    return {
      ...
      mediaPaths: (descriptions.meta?.mediaPaths || undefined) as Venue['mediaPaths'],
      videoUrl: descriptions.videoUrl || undefined,
...
export async function readPublicVenues(): Promise<Venue[]> {
  ...
  const venues: Venue[] = venuesDb.map((venue) => {
    ...
    return {
      ...
      mediaPaths: (descriptions.meta?.mediaPaths || undefined) as Venue['mediaPaths'],
      videoUrl: descriptions.videoUrl || undefined,
      reels: descriptions.reels?.map((reel, index) => ({ ...reel, venueId: reel.venueId || venue.id, order: Number.isFinite(Number(reel.order)) ? Number(reel.order) : index + 1 })) || [],
      menuUrl: String(descriptions.meta?.menuUrl || ''),
      menuPdfUrl: String(descriptions.meta?.menuPdfUrl || ''),
      openingHours: (() => {
        const hours = descriptions.meta?.openingHours as Venue['openingHours'];
...
export async function readAllData(): Promise<ConciergePayload> {
  ...
  const customers: Customer[] = customersDb.map((customer) => {
    ...
    return {
      ...
      notes: decodeBookingNotes(customerBookings[0]?.notes) || 'Không có ghi chú.',
